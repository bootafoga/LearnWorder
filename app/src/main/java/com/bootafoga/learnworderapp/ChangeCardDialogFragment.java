...
public class ChangeCardDialogFragment extends DialogFragment {
    ...
    private void changeWordByUserChoice(int position, int userChoise) {
        SQLiteDatabase db;
        Cursor cursor;
        DatabaseConnection connect = new DatabaseConnection(getLayoutInflater().getContext());
        ...
        if (userChoise == 0){ //edit
            ...
            db.delete("DICTIONARY", "_id = ?", new String[]{Integer.toString(cursor.getInt(0))});
            ...
            db.update("DICTIONARY", newVal, "_id = ?", new String[]{Integer.toString(cursor.getInt(0))});
            Intent intent = new Intent(getContext(), DictionaryActivity.class);
            intent.putExtra("delete", true);
            intent.putExtra("num_page",cursor.getInt(3));
            intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_NO_ANIMATION);
